...
All data sources are managed via BDP lockfile (bdp.lock)
...
exact versions are recorded in the BDP lockfile (bdp.lock) available
...
4. **Commit Lockfiles** - Store `bdp.lock` in version control alongside audit exports

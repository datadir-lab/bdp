...
- **Share exact configurations** with collaborators via `bdp.yml` and `bdp.lock`
...
4. **Collaborators get exact same data:**
   ```bash
   git clone <your-repo>
   bdp pull  # Downloads exact versions from bdp.lock
   ```
...
- [ ] Lockfile (`bdp.lock`) committed to version control

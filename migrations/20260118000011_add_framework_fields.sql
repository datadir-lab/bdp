...
ADD COLUMN records_skipped BIGINT DEFAULT 0;

-- Add missing fields to ingestion_raw_files
ALTER TABLE ingestion_raw_files
ADD COLUMN file_purpose VARCHAR(50),
ADD COLUMN computed_md5 VARCHAR(32);

-- Add missing fields to ingestion_work_units
ALTER TABLE ingestion_work_units
ADD COLUMN started_processing_at TIMESTAMPTZ,
...
CREATE INDEX ingestion_staged_records_job_idx ON ingestion_staged_records(job_id);
CREATE INDEX ingestion_staged_records_status_idx ON ingestion_staged_records(status);
CREATE INDEX ingestion_staged_records_type_idx ON ingestion_staged_records(record_type);
